...
async function basicLibrarySearch() {
    ...
    try {
        const libraries = await getContext7Client().resolveLibraryId(
            'React',
...
async function getBestLibraryMatch() {
    ...
    try {
        const bestLibrary = await getContext7Client().searchLibrary(
            'Express.js',
...
async function getLibraryDocumentation() {
    ...
    try {
        const docs = await getContext7Client().getDocumentation(
            'React',
...
async function expressIntegration() {
    ...
    app.get('/api/search-docs', async (req, res) => {
        try {
            ...
            const docs = await getContext7Client().getDocumentation(library, query);
...
async function compareLibraries() {
    ...
    try {
        ...
        const reactDocs = await getContext7Client().getDocumentation('React', 'component lifecycle');
        const vueDocs = await getContext7Client().getDocumentation('Vue', 'component lifecycle');
        ...
        const reactLibs = await getContext7Client().searchLibrary('React', 'component lifecycle');
        const vueLibs = await getContext7Client().searchLibrary('Vue', 'component lifecycle');
...
async function advancedUsage() {
    ...
    try {
        ...
        const tools = await getContext7Client().getTools();
        ...
        const initResult = await getContext7Client().initialize();
        ...
        const bestLib = await getContext7Client().searchLibrary('Express.js', query);

        if (bestLib) {
            console.log(`\n📚 Best match: ${bestLib.name}`);
            const docs = await getContext7Client().queryDocs(bestLib.id, query);
